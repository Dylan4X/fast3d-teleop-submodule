# Vendored from Fast-SAM-3D-Body/mocap/core/multiview_mhr2smpl.py
import inspect
import struct
...
class MultiViewFusionRunner:
    def __init__(
        ...
        self._baryc = np.asarray(mapping["baryc_coords"], dtype=np.float32)
        if "mhr_vert_ids" in mapping:
            self._mhr_vert_ids = np.asarray(mapping["mhr_vert_ids"], dtype=np.int64)
            ...
            self._mhr_vert_ids = faces[triangle_ids].astype(np.int64)
        else:
            raise RuntimeError(
