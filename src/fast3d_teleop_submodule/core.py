...
class Fast3DTeleopSubmodule:
    ...
    def __init__(self, config: Fast3DTeleopSubmoduleConfig):
        ...
        self._estimator = build_default_estimator(
            image_size=self.config.image_size,
            yolo_model_path=self._resolve_path(self.config.yolo_model_path),
            fov_model_size=self.config.fov_model_size,
            fov_resolution_level=self.config.fov_resolution_level,
            fov_fixed_size=self.config.fov_fixed_size,
            fov_fast_mode=self.config.fov_fast_mode,
        )
    ...
    def _discover_project_root(project_root: Optional[str]) -> Path:
        ...
        for candidate in candidates:
            ...
            if (candidate / "mocap").is_dir() and (candidate / "sam_3d_body").is_dir():
                return candidate
